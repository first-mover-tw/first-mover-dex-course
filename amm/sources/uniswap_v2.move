module amm::uniswapV2 {
    use std::{ascii, type_name::{Self, TypeName, with_defining_ids}};
    use sui::{
    ...
    public fun create_pool<A, B>(
        ...
        event::emit(PoolCreated {
            ...
            a: with_defining_ids<A>(),
            b: with_defining_ids<B>(),
    ...
    public fun add_liquidity<A, B>(
        ...
        let input_a_mul_pool_b: u128 =
            (balance::value(&input_a) as u128) * (balance::value(&pool.balance_b) as u128);
        let input_b_mul_pool_a: u128 =
            (balance::value(&input_b) as u128) * (balance::value(&pool.balance_a) as u128);

        let deposit_a: u64;
        let deposit_b: u64;
        let lp_to_issue: u64;
        if (input_a_mul_pool_b > input_b_mul_pool_a) {
            // input_a / pool_a > input_b / pool_b
    ...
    public fun remove_liquidity<A, B>(
        ...
        // return amounts
        (balance::split(&mut pool.balance_a, a_out), balance::split(&mut pool.balance_b, b_out))
    }
    ...
    public fun swap_a_for_b<A, B>(
        ...
        event::emit(Swapped {
            pool_id: object::id(pool),
            tokenin: with_defining_ids<A>(),
            amountin: input_amount,
            tokenout: with_defining_ids<B>(),
    ...
    public fun swap_b_for_a<A, B>(
        ...
        event::emit(Swapped {
            pool_id: object::id(pool),
            tokenin: with_defining_ids<B>(),
            amountin: input_amount,
            tokenout: with_defining_ids<A>(),
            amountout: out_amount,
        });
    ...
    fun destroy_zero_or_transfer_balance<T>(
        balance: Balance<T>,
        recipient: address,
        ctx: &mut TxContext,
    ) {
    ...
    fun test_cmp_type_names() {
        assert!(cmp_type_names(&with_defining_ids<BAR>(), &type_name::get<FOO>()) == 0, 0);
        assert!(cmp_type_names(&with_defining_ids<FOO>(), &type_name::get<FOO>()) == 1, 0);
        assert!(cmp_type_names(&with_defining_ids<FOO>(), &type_name::get<BAR>()) == 2, 0);

        assert!(cmp_type_names(&with_defining_ids<FOO>(), &type_name::get<FOOd>()) == 0, 0);
        assert!(cmp_type_names(&with_defining_ids<FOOd>(), &type_name::get<FOO>()) == 2, 0);

        assert!(cmp_type_names(&with_defining_ids<FOOD>(), &type_name::get<FOOd>()) == 0, 0);
        assert!(cmp_type_names(&with_defining_ids<FOOd>(), &type_name::get<FOOD>()) == 2, 0);
    }
    ...
    fun test_remove_pool_item<A, B>(factory: &mut Factory) {
        let a = std::type_name::with_defining_ids<A>();
        let b = std::type_name::with_defining_ids<B>();
